...
(function($) {
    function Plot(target_, data_, options_) {
       ...
       var timeUnitSize = {
        ...
        // gmurray : remove default rightMargin as it will get handled by y2axis
        var selectedPieSlice = null;
        var showCrosshairs = false;
        var crosshairX = null;
        var crosshairY = null;
        var explodeRadius = 0;
        if (options_.pie && options_.pie.explodeRadius > 0) {
         
            explodeRadius = options_.pie.explodeRadius;
        }        
        if (options_.rightMargin) {
            rightMargin = options_.rightMargin;
        }
        if (options_.leftMargin) {
            leftMargin = options_.leftMargin;
        }
        if (options_.topMargin) {
            topMargin = options_.topMargin;
        }
        if (options_.bottomMargin) {
            bottomMargin = options_.bottomMargin;
        }
        this.getMargins = function() {
            return { left : leftMargin, right : rightMargin, top : topMargin, bottom : bottomMargin};
        };
            ...
            for (var i = 0; i < series.length; ++i) {
                ...
                for (var j = 0; j < data.length; ++j) {
                    ...
                    if (y != null && !isNaN(y = +y)) {
                        ...
                        // gmurray
                        // track stacked barchart data
                        if (series[i].bars.stacked) {
                            if (!axisx.barstack) {
                                axisx.barstack = {};
                            }
                            if (!axisx.barstack[x]) {
                                axisx.barstack[x] = {height : 0 , data : [], outlineDrawn : false};
                            }
                            if (y) {
                                // set the top height to be the barsack height
                                if (!isNaN(y)) axisx.barstack[x].height += y;
                                if (axisx.barstack[x].height > axisy.datamax) {
                                    axisy.datamax = axisx.barstack[x].height;
                                }
                            }
        ...
        function constructCanvas() {
            // gmurray
            plotPosition = target.position();
            canvasWidth = target.width() - rightMargin - leftMargin;
            canvasHeight = target.height() - topMargin - bottomMargin;
            ...
            // the canvas
            // gmurray added leftMargin
            canvas = $('<canvas style="position:absolute;top:' + topMargin + 'px;left:' + leftMargin + 'px" width="' + canvasWidth + '" height="' + canvasHeight + '"></canvas>').appendTo(target).get(0);
            ...
            if (showCrosshairs) {
                crosshairX = $('<div style="height:1px;background:#FF0000;position:absolute;top:-100px;left:' + 0 + 'px;width:' + canvasWidth +'px"></div>').appendTo(target);
                crosshairY = $('<div style="width:1px;background:#FF0000;position:absolute;top:0px;left:-100px;height:' + canvasHeight +'px"></div>').appendTo(target);
            }
        }
        ...
        function insertLabels() {
            ...
            addLabels(axes.xaxis, function (tick, axis) {
                return '<div style="position:absolute;top:' + (plotOffset.top +   topMargin + plotHeight + options.grid.labelMargin) + 'px;left:' + (leftMargin + plotOffset.left + axis.p2c(tick.v) - axis.labelWidth/2) + 'px;width:' + axis.labelWidth + 'px;text-align:center" class="tickLabel">' + tick.label + "</div>";
            });
            
            //gmurray adding 25 to keep some room inside
            addLabels(axes.yaxis, function (tick, axis) {
                return '<div style="position:absolute;top:' + (plotOffset.top  + topMargin + axis.p2c(tick.v) - axis.labelHeight/2) + 'px;right:' + (rightMargin + plotOffset.right + plotWidth + options.grid.labelMargin) + 'px;width:' + axis.labelWidth + 'px;text-align:right" class="tickLabel">' + tick.label + "</div>";
            });
            
            addLabels(axes.x2axis, function (tick, axis) {
                return '<div style="position:absolute;bottom:' + (plotOffset.bottom  + bottomMargin + plotHeight + options.grid.labelMargin) + 'px;left:' + (leftMargin + plotOffset.left + axis.p2c(tick.v) - axis.labelWidth/2) + 'px;width:' + axis.labelWidth + 'px;text-align:center" class="tickLabel">' + tick.label + "</div>";
            });
            
            addLabels(axes.y2axis, function (tick, axis) {
                return '<div style="position:absolute;top:' + (plotOffset.top + topMargin + axis.p2c(tick.v) - axis.labelHeight/2) + 'px;left:' + (leftMargin + plotOffset.left + plotWidth + options.grid.labelMargin) +'px;width:' + axis.labelWidth + 'px;text-align:left" class="tickLabel">' + tick.label + "</div>";
            });
        ...
        function _drawSeriesPie(serie, lctx, arcStartAngle, arcEndAngle, bcolor, scolor, opacity, borderStrokeWidth) {
            if (!bcolor) {
                bcolor = serie.color;    
            }
            if (!scolor) {
                scolor = serie.pie.pieStrokeColor;    
            }
            // excanvas doesn't render 100% pie charts
            if ($.browser.msie && serie.percent == 100) {
                arcEndAngle -= .001;
            }
            var explode = {x : 0, y : 0};
            var halfAngle = (arcEndAngle + arcStartAngle)/2;
            
            // calculate the x, y direction offsets to explode to
            if (serie.explodeRadius) {
                explode.x = Math.round(Math.cos(halfAngle) * serie.explodeRadius);
                explode.y =  Math.round(Math.sin(halfAngle) * serie.explodeRadius);
            }            
            lctx.beginPath();
            lctx.lineWidth = 0;
            
            lctx.moveTo(pieCenterLeft + explode.x, pieCenterTop + explode.y);
            lctx.moveTo(pieCenterLeft + explode.x, pieCenterTop + explode.y); 

            lctx.arc(
                    // draw next arc
                    pieCenterLeft + explode.x, pieCenterTop + explode.y,
                    pieChartRadius, arcStartAngle, arcEndAngle, false);
            lctx.closePath();
            lctx.fillStyle = bcolor; // Color
            lctx.fill();

            // Draw the same thing again to draw stroke properly in IE
            if (serie.pie.pieStrokeLineWidth > 0) {
                lctx.strokeStyle = serie.pie.pieStrokeColor;
                lctx.lineWidth = borderStrokeWidth || serie.pie.pieStrokeLineWidth;
                lctx.beginPath();
                lctx.moveTo(pieCenterLeft + explode.x, pieCenterTop + explode.y); // Center of the pie
        ...
        function drawSeriesPie(serie){
            ...
            _drawSeriesPie(serie, ctx, arcStartAngle, arcEndAngle);
            ...
            // Draw pie labels
            if (typeof(options.pie.labelFormatter) == 'function') {
                var text = options.pie.labelFormatter(serie);
                
                var cwidth = 0;
                var cheight = 0;
                
                var label = $('<span  class="pieLabel" id="pieLabel'+ pieArcIndex+
                              '" style="visibility:hidden;position:absolute;top:' + (plotOffset.top + yh ) +
                              'px;left:' + (plotOffset.left + xh) + 'px;"><div>' + text + "</div></span>");
                target.append(label);
                label.css('opacity', serie.pie.labelBackgroundOpacity);
                
                // allow for any set of styles to be applied and override the defaults
                if (options.pie.labelStyles) {
                    for (var i in options.pie.labelStyles) {
                        if (typeof options.pie.labelStyles[i] == 'string') {
                            label.css(i, options.pie.labelStyles[i]);
                        }
                    }
                }
                cwidth = label.width();    
                cheight = label.height();
        
                var halfAngle = (arcEndAngle + arcStartAngle) / 2;
                
                var lo = serie.pie.labelOffset?serie.pie.labelOffset:pieChartRadius*serie.pie.labelOffsetFactor;

                var toffset = cwidth / 2;
                var loffset = cheight / 2;
        
                // offset if on the left
                if ( options.pie.labelStyles &&
                     options.pie.labelStyles.orientation == 'auto') {
                    if  (arcStartAngle <=  Math.PI / 2 + Math.PI && arcStartAngle >= Math.PI / 2 ) {
                        toffset = 0;
                        loffset = cwidth;
                    } else {
                        loffset = 0;
                ...
                var xh = pieCenterLeft - loffset + Math.round(Math.cos(halfAngle) * lo) + explode.x;
                var yh = pieCenterTop - toffset  + Math.round(Math.sin(halfAngle) * lo) + explode.y;
    
                
                label.css('top', (plotOffset.top + yh  ));
                label.css('left', (plotOffset.left + xh));
                // make sure click events go through
                label.click(onClick);
            
                // end gmurray
                if (options.pie.showLabel) {
        ...
        function drawBar(x, y, barLeft, barRight, offset, fill, axisx, axisy, c, series, index, _overlay) {
            ...
            // stack it
            if (axisx.barstack && axisx.barstack[x]) {
                var _voffset = 0;
                stacked = true;    
                for (var i=0; i < axisx.barstack[x].data.length;i++) {
                    if (axisx.barstack[x].data[i].series !== series.id) {
                        _voffset += axisx.barstack[x].data[i].y;
                           if (!series.voffsets) {
                            series.voffsets = {};
                        }
            ...
            if (index === 0) {
              ...
              var _right = axisx.p2c(right) + barRight - 1;
            var _left = axisx.p2c(left) + barLeft + 1 ;
            var _bottom = axisy.p2c(bottom);
            var _top = axisy.p2c(top);
            ...
            // gmurray - down't allow for overflow
            if (_left <= 1 ) {
                  _left = 1;
            }
            if (_right >=  plotWidth) {
            ...
            if (overlay) {
                ...
                padding= { l: (!stacked || firstLeft) ? 0 : 1, r: (!stacked || lastRight) ? 0 : 1};
            }
            ...
            if (fill) {
                ...
                c.moveTo(_left +  padding.l + offsetLeft, _bottom );
                c.lineTo(_left + padding.l  + offsetLeft, _top + offset);
                c.lineTo(_right - padding.r , _top + offset);
                c.lineTo(_right - padding.r, _bottom  + offset);
            ...
            if (drawLeft || drawRight || drawTop || drawBottom) {
                ...
                c.moveTo(_left  + offsetLeft  + _lw , _bottom + offset);
                
                if (drawLeft)
                    c.lineTo(_left + offsetLeft + _lw, _top + offset);
                else
                    c.moveTo(_left + offsetLeft + _lw, _top + offset);
                    
                if (drawTop) {
                    c.lineTo(_right - c.lineWidth, _top + offset);
                } else {
                    c.moveTo(_right - c.lineWidth, _top + offset);
                }
                
                if (drawRight)
                    c.lineTo(_right - 1, _bottom + offset);
                else
                    c.moveTo(_right - 1, _bottom + offset);
                    
                if (drawBottom) {
                    c.lineTo(_left + offsetLeft, _bottom + offset);
                } else {
                    c.moveTo(_left + offsetLeft, _bottom + offset);

                }
                c.stroke();
            }

               if (axisx.barstack && axisx.barstack[x].outlineDrawn == false) {
          
                // add one px to contain everything. 
                var _height = axisy.p2c(axisx.barstack[x].height) - 1;
                c.strokeStyle = "#000";
                c.beginPath();     
                ...
                if (!firstLeft) {
                    c.lineTo(_left  + offsetLeft + _lw, _height + offset);
                } else {                   
                    c.moveTo(_left -1 + offsetLeft + _lw, _height + offset);
                }
                if (!lastRight) {
                    c.lineTo(_right - c.lineWidth , _height + offset);
                } else {
                    c.lineTo(_right , _height + offset);
                }
                if (!lastRight) {
                    c.lineTo(_right - c.lineWidth, _bottom + offset);
                }
               c.stroke();
        ...
        function drawSeriesBars(series) {
            function plotBars(data, barLeft, barRight, offset, fill, axisx, axisy, _series, i) {
                for (var i = 0; i < data.length; i++) {
                    if (data[i] == null)
                        continue;
                    drawBar(data[i][0], data[i][1], barLeft, barRight, offset, fill, axisx, axisy, ctx, _series, i);
                }
            }
            ...
            ctx.strokeStyle = series.color;
            setFillStyle(series.bars, series.color, 0.9);
            var barLeft = series.bars.align == "left" ? 0 : -series.bars.barWidth/2;
         
            var bw = series.bars.barWidth;
            plotBars(series.data, barLeft - (bw / 2), barLeft + bw  - (bw / 2), 0, series.bars.fill, series.xaxis, series.yaxis, series);
            ctx.restore();
        }

        function setFillStyle(obj, seriesColor, _opacity) {
            var opacity = _opacity || 0.4
            var fill = obj.fill;
            if (!fill)
                return;
            ...
            if (obj.fillColor)
                ...
                c.a = typeof fill == "number" ? fill : opacity;
                c.normalize();
        ...
        function insertLegend() {
            ...
            var sorted = [];
            var tdStyle = "padding-right:2px";
           if (options.legend.width == 'auto' &&
               options.legend &&
               options.legend.noColumns > 1 && series.length > 1) {

               tdStyle = "width:" + (100 / options.legend.noColumns) + "%";
            }
            // gmurray modified to allow for sorted pie chart elements.
            // later this could be modified for alphabetical listing of series names
            for (i = 0; i < series.length; ++i) {
                if (!series[i].label)
                    continue;

                var label;
                if (typeof options.legend.labelFormatter == 'function') {
                    label = options.legend.labelFormatter(series[i].label, series[i]);
                } else {
                    label = series[i].label;
                }
                sorted.push({
                series : series[i],
                // gmurray changed to allow for lookup and printing
                content :   '<td class="legendColorBox"><div style="border:1px solid ' + options.legend.labelBoxBorderColor + ';padding:1px"><div style="width:14px;height:10px;background :' + series[i].color + ';overflow:hidden"></div></div></td>' +
                            '<td  style="padding-left:2px;' + tdStyle+'" class="' + ((series[i].selectable === false) ? 'legendLabel' : 'legendSelectable') +
                            '" id="' + options.legend.legendBase + '_legend_' + series[i].id + '">' + label + '</td>'
                });

            }

            //if we are a pie chart sort based on percentage.
            if (options.pie.show) {
                sorted.sort(function(a,b) {

                    if (a.series.percent == b.series.percent) {
                        return 0;
                    } else {
                        return  (a.series.percent > b.series.percent) ? -1 : 1;
                    }
                } );
            }
            ...
            for (i = 0; i < sorted.length; i++) {
                ...
                var label = sorted[i].series.label;
                if (options.legend.labelFormatter != null)
                    label = options.legend.labelFormatter(label, sorted[i].series);
                fragments.push(sorted[i].content);
            ...
            if (options.legend.container != null)
                ...
                if (p.charAt(0) == "n")
                    pos += 'top:' + (m + plotOffset.top + topMargin) + 'px;';
            ...
            // gmurray - wire in the selection listeners 
            if (options.legend.container != null ){
               options.legend.container.find(".legendSelectable").each(function() {
                   this.style.cursor = "pointer";
                   this.style.textDecoration = "underline";
                   this.onclick = legendSelect;
               });
            }
        }
        ...
        function findNearbyItem(mouseX, mouseY) {
            ...
            for (var i = 0; i < series.length; ++i) {
                ...
                // gmurray TODO : handle pie stuff
                if ( series[i].pie && series[i].pie.show === true) {
                    continue;
                }

                var axisx = series[i].xaxis,
                    axisy = series[i].yaxis;

                    // gmurray - account for time series where the bar width needs scaling
                    var barWidth = series[i].bars.barWidth;
                    
                    if (axisx.mode == 'time') {
                        barWidth = axisx.c2p(barWidth) - axisx.min;
                    }
                    ...
                    var barLeft = 0;//series[i].bars.align == "left" ? 0 : -barWidth/2,
                    var barRight = ( barLeft +  halfBar); // series[i].bars.align == "left" ? barLeft + barWidth : ( barLeft + barWidth/2);

                    // gmurray
                    checkpie = (series[i].pie && series[i].pie.show);
                ...
                for (var j = 0; j < data.length; ++j) {
                    ...
                    var voff = 0;
                    if (series[i].voffsets && typeof series[i].voffsets[j] == 'number' ) {
                        voff = series[i].voffsets[j];
                    }
                    ...
                    if (checkbar) {
                        ...
                        if (!foundPoint &&
                           ...
                           my >= Math.min(voff, y) && my <= Math.max(voff, y)
                            ) {
                       
                            item = result(i, j, voff);
        ...
        function drawPieOverlay(serie, arcStartAngle, arcEndAngle) {
            redrawTimeout = null;
            if (serie.selectable === false) {
            ...
            _drawSeriesPie(serie, octx, arcStartAngle, arcEndAngle, '#FFF', '#FF0000', 5);
        ...
        /*
         * Calculate the pie slice that the mouse may be over
         */
        function getPie(mouseX, mouseY) {

            selectedPieSlice = null;

            var pieTray = 0;      
             
            var a = pieCenterLeft - mouseX;
            var b = mouseY - pieCenterTop;
                        
            // now calculate the angle of the current point and see if it is bounds
            var angle = (Math.atan(b/a));
            
            var distance = (Math.sqrt((b*b)+ (a *a) ));
            
            // if we run outside the radius bug out
            if (distance > pieChartRadius ) {
                clearPieSelection();
                   return;
            }
            ...
            // if we are between -PI/2 and 0
            if (mouseX >= pieCenterLeft && mouseY <= pieCenterTop) {
                angle = angle * -1;
             }
            // if we are between 0 and PI/2
            else if (mouseX >= pieCenterLeft && mouseY > pieCenterTop ){
                 angle = angle * -1;
            }
            // if we are between PI/2 and PI
            else if (mouseX <= pieCenterLeft && mouseY > pieCenterTop ){
                 angle = Math.PI - angle;
            }            
            // if we are between PI and  -PI/2
            else if (mouseX <= pieCenterLeft && mouseY <= pieCenterTop ){
                 angle = Math.PI - angle; + Math.PI;
            }

            
            // gmurray : TODO - remove this debugging code
            //var an = document.getElementById("debug");
            //if (an) an.innerHTML = "angle is " + angle + " distance=" + distance;
            ...
            for (var i = 0; i < series.length; ++i) {
                ...
                var arcStartAngle = Math.PI * (- 0.5 + 2 * pieTray); // -0.5 sets set the start to be top
                var arcEndAngle = Math.PI * (- 0.5 + 2 * (pieTray + thisvalue));
        
                if (angle >= arcStartAngle && angle < arcEndAngle ) {
                    if (selectedPieSlice != serie.id) {
                        drawPieOverlay(serie, arcStartAngle, arcEndAngle);
                            pieSelectedSlice = serie;
                       }
                }
                // now check if the angle is between 
                
                pieTray += thisvalue; // increment progress tracker
            }
        }
        ...
        function onLegendSelect(e, item) {
            
        }
        
        // gmurray
        function legendSelect(e) {
            // get the id
            if (window.event) e = window.event; 
            var source = e.target || e.srcElement;
            // the dataset id is what follows the _legend_
            var _index = source.id.indexOf("_legend_");

            if (_index < 0) {
                return;
            }
            var seriesId = source.id.substring(_index + "_legend_".length , source.id.length);

            if (seriesId) {
                for (var i=0; i < series.length;i+=1) {
                    if (series[i].id === seriesId && series[i].selectable !== false) {
                        target.trigger("legendselect", [ series[i], source.id ]);
                        break;
                    }
                }
            }
        ...
        function triggerClickHoverEvent(eventname, event) {
            ...
            if (item) {
                // account for vertical offset of stacked bars
                var voff = item.voffset || 0;
                item.pageX = parseInt(item.series.xaxis.p2c(item.datapoint[0]) + offset.left + plotOffset.left);
                item.pageY = parseInt(item.series.yaxis.p2c(item.datapoint[1] + voff) + offset.top + plotOffset.top );
            }
            ...
            if (options.grid.autoHighlight) {
                ...
                if (item)
                    highlight(item.series, item.datapoint, true, item.dataIndex);
            }
            getPie(canvasX, canvasY);
            if (!item && pieSelectedSlice) {
                item = pieSelectedSlice;
            }
            // gmurray : crosshairs      
            if (showCrosshairs) {    
                crosshairY.css("left",  canvasX + "px");
                crosshairX.css("top",canvasY + "px");
           }
            if (item && options.legend) {
                item.legendElementId = options.legend.legendBase + '_legend_' + item.id;
            }
        ...
        function redrawOverlay() {
            ...
            for (i = 0; i < highlights.length; ++i) {
                ...
                if (hi.series.bars.show)
                    drawBarHighlight(hi.series, hi.point, hi.index);
                else
        ...
        function highlight(s, point, auto, index) {
            ...
            if (i == -1) {
                highlights.push({ series: s, point: point, auto: auto, index : index });
        ...
        function drawBarHighlight(series, point, index) {  
            ...
            drawBar(point[0], point[1], barLeft + series.bars.barWidth/2, barLeft - series.bars.barWidth /2,
                   0, true, series.xaxis, series.yaxis, octx, series, index, true);            
        }
